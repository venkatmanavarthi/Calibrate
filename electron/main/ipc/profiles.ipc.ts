...
import type { ExportBundle, ExperienceProfile } from '../../../src/types/models'
import { PROFILES_DIR } from '../storage/index'
...
export function registerDataIpc(win: BrowserWindow): void {
  ...
  ipcMain.handle('import:data', async () => {
    ...
    if (result.canceled || !result.filePaths[0]) {
      return { imported: false, profileCount: 0 }
    }

    const raw = await fs.readFile(result.filePaths[0], 'utf-8')
    const bundle = JSON.parse(raw) as ExportBundle

    if (bundle.version !== 1 || !Array.isArray(bundle.profiles)) {
      throw new Error('Invalid export file format')
    }

    await Promise.all(bundle.profiles.map((p: ExperienceProfile) =>
